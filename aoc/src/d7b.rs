...
pub fn d7b() -> io::Result<()> {
    ...
    let total_used: i32 = *sizes.get("/").unwrap() as i32;
    let free_space: i32 = 70000000 - total_used;
    let add_space_needed: i32 = 30000000 - free_space;

    let result: u32 = *sizes
        .iter()
        .filter(|&(_, v)| (*v as i32) >= add_space_needed)
        .map(|(_, v)| v)
        .min()
